...
- 测试：`packages/psycho-frame/test/init.test.mjs` 新增三条入口、依赖提示与路径形式回归断言。
...
- 测试：84/84 通过，含新增的模板入口断言、doctor 依赖提示断言与路径形式回归断言。
...
- 合流后回归：`doctor` 的依赖提示只在脚本调用裸 `psycho-frame` 时触发；框架仓库与 workspace
  的路径形式（`node packages/psycho-frame/src/cli.mjs verify`）不再误报 devDependency 缺失。
...
- 官网内容由 `README.md`、`docs/cookbook/tutorial.md`、`docs/concepts.md`、包 README 生成，
  本次改到其中两个源，`pnpm docs:build` 已复跑通过（vitepress 1.6.4，4 个页面）。
