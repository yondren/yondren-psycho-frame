...
test('validateWorkMode：非对象非法', () => {
  ...
  assert.match(validateWorkMode('on')[0], /必须为 \{ plan, confirmAmbiguous, fleet, merge \} 对象/)
})

test('validateWorkMode：取值封闭', () => {
  assert.deepEqual(validateWorkMode({ plan: 'off', confirmAmbiguous: false, fleet: 'on', merge: 'auto' }), [])
...
test('setWorkMode：写入并可增量合并', () => {
  ...
  assert.deepEqual(first.workMode, { plan: 'off', confirmAmbiguous: true, fleet: 'off', merge: 'ask' })
  const second = setWorkMode(root, { confirmAmbiguous: false })
  assert.equal(second.ok, true)
  assert.deepEqual(second.workMode, { plan: 'off', confirmAmbiguous: false, fleet: 'off', merge: 'ask' })
  assert.deepEqual(readWorkMode(root), { plan: 'off', confirmAmbiguous: false, fleet: 'off', merge: 'ask' })
})
